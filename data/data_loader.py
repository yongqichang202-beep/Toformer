...
class Dataset_ETT_day(Dataset):
    ...
    def __read_data__(self):
        ...
        df_raw = df_raw[(df_raw['date'].dt.hour == 0) & (df_raw['date'].dt.minute == 0)]

       
        num_days = len(df_raw)
        num_train = int(0.7 * num_days)
        num_val = int(0.1 * num_days)
        num_test = num_days - num_train - num_val

        border1s = [0, num_train - self.seq_len, num_train + num_val - self.seq_len]
        border2s = [num_train, num_train + num_val, num_days]

        border1 = border1s[self.set_type]
        border2 = border2s[self.set_type]

        if self.features in ['M', 'MS']:
            cols_data = df_raw.columns[1:] if self.cols is None else self.cols
            df_data = df_raw[cols_data]
        else: 
            df_data = df_raw[[self.target]]
        ...
        data_stamp = time_features(df_stamp, timeenc=self.timeenc, freq='d')
